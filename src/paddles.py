...
class Paddles():
    def __init__(self) -> None:
        ...
        self.paddle_1_rect = self.paddle.get_rect(
            midtop=(40, self.paddle_1_posy))

        self.paddle_2_rect = self.paddle.get_rect(
            midtop=(760, self.paddle_2_posy))
    ...
    def paddles_movement(self, data: data.Data) -> None:
        ...
        if keys[pygame.K_s] and self.paddle_1_rect.y + \
                self.paddle_height <= data.game_height:
            self.paddle_1_rect.y += self.paddles_speed

        if keys[pygame.K_UP] and self.paddle_2_rect.y >= 0:
            self.paddle_2_rect.y -= self.paddles_speed
        if keys[pygame.K_DOWN] and self.paddle_2_rect.y + \
                self.paddle_height <= data.game_height:
            self.paddle_2_rect.y += self.paddles_speed
