...
class Game(Game_strategy):
    ...
    def build_game(
        ...
        def game_objects() -> None:
            ...
            # speed_clock
            speed_text = data.font_clock.render(
                "Speed: " + str(data.clock_speed), True, "White")
        ...
        def game_logic() -> None:
            if ball.ball_rect.y == data.game_height - ball.ball_size:
                ball.move_y = -1
            # right side
            if ball.ball_rect.x == data.game_width - ball.ball_size:
            ...
            if ball.ball_rect.x == 50 and (
                ball.ball_rect.y <= (
                    paddles.paddle_1_rect.y +
                    paddles.paddle_height)) and ball.ball_rect.y >= (
                paddles.paddle_1_rect.y -
                    ball.ball_size):
                ball.move_x = 1

            if ball.ball_rect.x == 730 and (
                ball.ball_rect.y <= (
                    paddles.paddle_2_rect.y +
                    paddles.paddle_height)) and (
                ball.ball_rect.y >= (
                    paddles.paddle_2_rect.y -
                    ball.ball_size)):
                ball.move_x = -1
...
class Main():
    ...
    def build_program(self) -> None:
        while True:
            ...
            self.game_strategy.build_game(
                self.data, self.paddles, self.ball)  # game_strategy
            ...
            if event.type == pygame.QUIT:
                ...
                if 150 <= event.pos[0] <= 250 and 200 <= event.pos[1] <= 250:
                    ...
                    self.data.screen = pygame.display.set_mode(
                        (self.data.game_width,
                         self.data.game_height +
                         100))  # 800x400 - game, 800x100 - bottom panel
                if 150 <= event.pos[0] <= 250 and 300 <= event.pos[1] <= 350:
                    pygame.quit()
                    exit()
            elif event.type == pygame.MOUSEBUTTONDOWN and str(self.game_strategy) == "Game":
                # menu_button
                if 475 <= event.pos[0] <= 575 and 425 <= event.pos[1] <= 475:
                    self.game_strategy = Menu()
                    self.data.screen = pygame.display.set_mode(
                        (self.data.game_height, self.data.game_width))
                    self.data.game_mode = 0
                # exit_button
                if 350 <= event.pos[0] <= 450 and 425 <= event.pos[1] <= 475:
                    pygame.quit()
                    exit()
                # restart_button
                if 225 <= event.pos[0] <= 325 and 425 <= event.pos[1] <= 475:
                    self.data.game_mode = 0
